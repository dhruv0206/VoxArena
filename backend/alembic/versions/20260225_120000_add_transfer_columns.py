...
down_revision: Union[str, None] = 'c3f8a2b1d4e6'
...
def upgrade() -> None:
    ...
    transfertype = sa.Enum('WARM', 'COLD', name='transfertype')
    transfertype.create(op.get_bind(), checkfirst=True)

    op.add_column('voice_sessions', sa.Column('transferred_to', sa.String(length=50), nullable=True))
    op.add_column('voice_sessions', sa.Column('transfer_type', transfertype, nullable=True))
