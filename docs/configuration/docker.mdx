...
The database password is configured via `POSTGRES_PASSWORD` in `backend/.env`:
...
Both the PostgreSQL container and the backend read this variable from `backend/.env` via Docker Compose's `env_file` directive. The backend uses it to construct `DATABASE_URL` automatically.
