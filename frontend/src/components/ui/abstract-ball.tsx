...
const AbstractBall: React.FC<AbstractBallProps> = ({
    ...
    useEffect(() => {
        ...
        // Clean up any existing renderer (React Strict Mode protection)
    ...
    useEffect(() => {
        if (cameraRef.current) {
            gsap.to(cameraRef.current.position, { duration: 2, z: 300 - cameraZoom });
        }
    }, [cameraZoom]);

    // Update visibility when spherePoints changes
    useEffect(() => {
        if (meshRef.current && pointRef.current) {
            meshRef.current.visible = !spherePoints;
            pointRef.current.visible = spherePoints;
        }
    }, [spherePoints]);
