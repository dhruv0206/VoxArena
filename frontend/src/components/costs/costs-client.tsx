    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            const [summaryRes, providersRes, timelineRes, agentsRes] = await Promise.all([
                fetch(`${apiUrl}/dashboard/costs/summary?${params}`, { headers, cache: "no-store" }),
                fetch(`${apiUrl}/dashboard/costs/by-provider?${params}`, { headers, cache: "no-store" }),
                fetch(`${apiUrl}/dashboard/costs/timeline?${params}&granularity=${granularity}`, { headers, cache: "no-store" }),
                fetch(`${apiUrl}/dashboard/costs/by-agent?${params}&sort_by=${sortField}&sort_dir=${sortDir}`, { headers, cache: "no-store" }),
            ])
